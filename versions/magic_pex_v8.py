...
# FIXED VERSION v8 (Estabilizada para Magic 8.3.683 en WSL)
#
# Soluciones implementadas:
# 1. Búsqueda recursiva de .magicrc basada en la tecnología activa.
# 2. Protección contra Segmentation Fault en WSL (ulimit -s unlimited).
# 3. Flujo clásico de extracción Tcl (flatten -> ext2sim -> extresist all).
# 4. Ajuste de umbrales en ext2spice para evitar errores de división por cero.
...
# ── Buscar magicrc de forma recursiva ────────────────────────
def find_magicrc(pdk_dir):
    matches = []
    for root, dirs, files in os.walk(pdk_dir):
        for file in files:
            if file.endswith(".magicrc") and "_local_local" not in file:
                matches.append(os.path.join(root, file))
    
    local_rc = [f for f in matches if "_local.magicrc" in os.path.basename(f)]
    originals = [f for f in matches if "_local" not in os.path.basename(f)]
...
# 2. EXTRACCIÓN CAPACITIVA
extract all

# 3. GENERACIÓN DE MAPA DE NODOS
ext2sim labels on
ext2sim

# 4. EXTRACCIÓN RESISTIVA
extresist threshold 1
extresist minres 1
extresist simplify off
extresist all

# 5. VOLCADO A SPICE (Instrucciones individuales estables)
ext2spice format spice3
ext2spice cthresh 0.01
ext2spice rthresh 0.01

# Activar la lectura del archivo .res.ext de forma explícita
ext2spice extresist on

# Desactivar la combinación de dispositivos para mantener la fidelidad
ext2spice merge none

# Escritura final del netlist parásito
...
def run_magic(tcl_script, magicrc, work_dir):
    ...
    # VITAL: Ejecutamos a través de bash y quitamos el límite de stack de WSL 
    # (ulimit -s unlimited) para evitar el Segmentation Fault durante extresist
    # al iterar sobre la malla aplanada de metales.
    cmd_bash = f"ulimit -s unlimited || ulimit -s 65536; magic -rcfile {magicrc_wsl_safe} -noc -dnull {tcl_wsl_safe}"
...
def main():
    ...
    if os.path.exists(salt_dir):
        clean_tech = re.sub(r"[^A-Za-z0-9]", "", tech_name).lower() if tech_name else ""
        
        # Recopilar carpetas y asignarles puntuación según coincidan con la tecnología activa
        candidate_folders = []
        for f in os.listdir(salt_dir):
            folder_path = os.path.join(salt_dir, f)
            if os.path.isdir(folder_path):
                folder_clean = re.sub(r"[^A-Za-z0-9]", "", f).lower()
                score = 0
                if tech_name and tech_name != "(Default)":
                    if clean_tech == folder_clean:
                        score = 3  # Coincidencia exacta
                    elif clean_tech in folder_clean or folder_clean in clean_tech:
                        score = 2  # Coincidencia parcial
                candidate_folders.append((score, folder_path))
        
        # Ordenar de mayor a menor puntuación para priorizar la tecnología seleccionada
        candidate_folders.sort(key=lambda x: x[0], reverse=True)
        
        for score, folder_path in candidate_folders:
    ...
    if not pdk_dir or not magicrc_original:
        pya.MessageBox.warning(
            "Magic PEX", 
            f"No se encontró .magicrc válido en salt.\n(Tecnología actual: {tech_name})", 
            pya.MessageBox.Ok
        )
        return
    ...
    # ============================================================
    # DEPURACIÓN: Comprobar la cadena de archivos
    # ============================================================
    flat_cell = f"{cell_name}-pex"
    ext_file = os.path.join(work_dir, f"{flat_cell}.ext")
    res_ext_file = os.path.join(work_dir, f"{flat_cell}.res.ext")
    
    ext_ok = os.path.exists(ext_file)
    res_ok = os.path.exists(res_ext_file)
    
    print(f"[Magic PEX Debug] Extracción estándar (.ext): {'ÉXITO' if ext_ok else 'FALLÓ'} -> {ext_file}")
    print(f"[Magic PEX Debug] Extracción resistiva (.res.ext): {'ÉXITO' if res_ok else 'FALLÓ'} -> {res_ext_file}")
